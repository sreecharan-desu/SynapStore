...
/**
 * POST /api/v1/email/storeowner-dispatch
 * Public (No Auth) Endpoint to send a stock dispatch notification email to a store owner.
 */
emailRouter.post("/storeowner-dispatch", async (req, res) => {
...
/**
 * POST /api/v1/email/supplier-delivery-failed
 * Public (No Auth) Endpoint to notify a supplier that an email to a store failed.
 */
emailRouter.post("/supplier-delivery-failed", async (req, res) => {
  const requestId = `REQ-${Date.now()}`;
